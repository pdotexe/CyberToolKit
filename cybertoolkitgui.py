...
class CyberToolkitGUI(QWidget):
    ...
    def check_password_strength(self):
        ...
        if ok and password:
            ...
            if re.search(r"\d", password): score += 3
            if re.search(r"[!@#$%^&*(),.?\":{}|<>]", password): score += 4
    ...
    def handle_hash_checker(self):
        ...
        if not ok or not algo:
            self.status.setText("No algorithm selected.")
            return
    ...
    def handle_port_scanner(self):
        ...
        protocol, ok = QInputDialog.getItem(self, "Enter Protocol", "Choose a protocol:", ["TCP", "UDP"], 0, False)
